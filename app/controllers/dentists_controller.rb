class DentistsController < ApplicationController
  ...
  def create
    ...
    if @dentist.save!
      @user = User.new(
        email: params[:dentist][:email], password: ENV['temp_pass'],
  ...
  private
    ...
    def dentist_params
      params.required(:dentist).permit(:name, :office_name, :email, :phone, :address, :address_state, :address_zip, :address_city, :license_num, :notes, :tax_rate)
    end
end
